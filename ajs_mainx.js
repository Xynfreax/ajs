    ...
    main:{
      set:function(v){
        ...
        if($.hasOwnProperty(v)){
          ...
          // alert($[v].dom.tagName);
          document.body.children[0].appendChild($[v].dom)
...
//        alert(this.nCompleted);
    ...
    addChild_r:{configurable:true,writable:true,enumerable:false,value:function(obj_,indice){
      ...
      // obj_.parent=this;
      let obj=crear_objeto_js(obj_,this);
...
function crear_objeto_js(json_l_,parent_){
  ...
  if(json_l.tag!==undefined){
    ...
    //|Si no hay ningun problema
      ...
      if($[tag]!==undefined){
        ...
        // new_obj=heredar_de($[tag],json_l);
        new_obj=extenderObj($[tag].json_l);
      ...
      let anexar_=undefined;
      if(parent_){
        new_obj.parent=parent_;
      }
      if(new_obj.parent && new_obj.parent.dom){
        anexar_=new_obj.parent.dom;
      }
      // alert(JSON.stringify(new_obj.json_l));

      // alert(json_l.aqmodel);
      new_obj.json_l=json_l;

      renderizar(new_obj,anexar_,json_l);
      // alert(json_l.aqmodel);
      // if(json_l.hasOwnProperty('children')){
      if(json_l.children!==undefined && esherencia==false){
        alert('msg');
        // alert('en children: '+json_l.children);
        // alert($[tag]);
        // alert(JSON.stringify(json_l_));
        //--console.log(json_l.children[0]);
        let tamano_=new_obj._children.length;
        let tamano_2=json_l.children.length;
        for(let i=0;i<tamano_;i++){
          ...
          let child_=new_obj._children[i];
...
//--ineridp
    if(obj.json_l && obj.json_l.innerIdP!==undefined){
...
//--      console.log(obj.getParentBy.toString());
    ...
    if(obj.json_l && obj.json_l.idf!==undefined){
      var nominparent=obj.json_l.idf;
      //--->var parent_=obj.getParentBy('real_tag','form');
      var parent_=obj.getParentBy('real_tag',gltipopadre);
      if(parent_!==null){
        //--obj.classList.add('innerIdF_'+nominparent);
        obj.classList.add(nominparent);
        parent_[nominparent]=obj;
      }else{
        alert('Error when using innerIdF: '+obj.json_l.idf);
      }
    }
...
// alert(JSON.stringify(obj));
  obj.dom['id']=obj['id'];
    ...
    //------------PREINIT
        ...
        var actObj=this_;
        var arObj=[];
        while(actObj!==undefined && actObj.json_l!==undefined && actObj['preinit']!==undefined){
          if(actObj.json_l.hasOwnProperty('preinit')){
            //--actObj.load_r.bind(this_)();
            //console.log('-en for');
            //console.log(actObj.json_l.load_r.toString());
            if(arObj[0]!==actObj.json_l.preinit){
              arObj.unshift(actObj.json_l.preinit);
            }
    ...
    //------------INIT
        ...
        var actObj=this_;
        var arObj=[];
        while(actObj!==undefined && actObj.json_l!==undefined && actObj['init']!==undefined){
    ...
    //FIN INIT---------------------------------------
      ...
      //if(obj.children.length==obj.childrenLoaded){//ejecuta el load_r si sus hijos se han cargado
        ...
        while(actObj!==undefined && actObj.json_l!==undefined && actObj.json_l['load_r']!==undefined){
          ...
          if(actObj.json_l.hasOwnProperty('load_r')){
            //---arObj.push(actObj.json_l.load_r);
            //-----arObj.unshift(actObj.json_l.load_r);
            if(arObj[0]!==actObj.json_l.load_r){
              arObj.unshift(actObj.json_l.load_r);
            }
...
///---  new_obj._children=obj._children.slice(0);//|Clonamos el array
  ...
  new_obj.json_l=extenderObj(obj.json_l);
  ...
  if(json_l.assignProps!==undefined){
    ...
    new_obj.json_l.assignProps=Object.extenderObj(json_l.assignProps);
  }
  // new_obj.id=json_l.id;//esto lo modifiqué recién
  //new_obj.id=obj.id;
  new_obj.tag=json_l.tag;
  new_obj.classN=new_obj.classN+' '+obj.id;
  ...
  for(var i=0;i<0/*new_obj._children.length*/;i++){
    var json_l_for_hijo={};
...
function renderizar(obj,anexar,json_l){
...
// }
  ...
  if(obj.dom==obj.dom /*&& anexar.querySelector('[id="'+obj.id+'"]')==null*/){
    // alert(obj.json_l.aqmodel);
    // alert(obj.anexar);
